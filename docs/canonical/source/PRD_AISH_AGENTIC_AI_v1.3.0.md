...
**Versi:** 1.3.0  
**Status:** STEP 4 DOMAIN/BRANDING/ENVIRONMENT/SAAS-FOUNDATION PLANNING BASELINE COMPLETE — PRODUCT IMPLEMENTATION NOT STARTED  
**Tanggal:** 13 Juli 2026  
**Timezone:** Asia/Makassar  
**Pemilik produk:** Aish Tech Solution  
**Sumber kanonik:** Master Source Aish Agentic AI v2.4.0  
...
# 31. STEP 4 — DOMAIN, BRANDING, ENVIRONMENT, AND SAAS FOUNDATION PLANNING REQUIREMENTS

Canonical source: Master Source v2.4.0 §68. This section adds **planning** requirements only; it does not change
the MVP scope (§10) or persona baseline (§28). **Product implementation: NOT STARTED.**

- **Domain & URL:** the product name is "Aish Agentic AI" (unchanged); a preferred primary domain
  (`aishagentic.ai`) plus fallbacks MUST be recorded with point-in-time availability evidence; ownership MUST NOT
  be claimed without registration evidence; canonical subdomains and exact-match per-environment OAuth redirect
  URIs MUST be defined; email MUST enforce SPF/DKIM/DMARC. (`docs/domain/*`)
- **Brand:** branded-house hierarchy, official descriptor, a working tagline (APPROVED WORKING BASELINE), brand
  voice, and accessible (WCAG 2.2 AA) planning tokens that MUST NOT be claimed implemented in UI. (`docs/brand/*`)
- **Environment:** six environments (local/test/CI/staging/pilot/production) with database/redis/queue/storage
  isolation, synthetic-default data policy (no production data in local/test/CI/staging), per-environment secrets,
  and promotion gating with no direct unreviewed pilot/production deploy. (`docs/environments/*`)
- **Configuration & secrets:** every configuration item MUST be classified; secrets MUST NOT be committed.
- **Dependencies:** Laravel 12 / PHP 8.4 / PostgreSQL 17 / Redis 7.x baseline researched against official
  sources; no package installed; supply-chain controls (official registry, typosquat prevention, SBOM). (`docs/dependencies/*`)
- **SaaS Foundation:** a sequenced implementation plan (16 epics, 9 sprints), Definition of Ready/Done, test &
  evidence plan, deployment-target class, and rollback plan; first sprint SPRINT-SF-00. (`docs/planning/*`)
- **Non-functional (planning):** tenant isolation, security, and truthful-state requirements from §15/§16 carry
  into every Step 4 baseline; nothing is deployed, owned, or installed.
- **Definition of Ready / Release criteria:** SaaS Foundation implementation MUST NOT start before the Step 4
  Definition of Ready is satisfied and the Step 4 GO tag is verified (`docs/planning/SAAS_FOUNDATION_DEFINITION_OF_READY.md`,
  `docs/quality/STEP_4_GO_WATCH_NO_GO.md`).

# 32. CHANGELOG

## Version 1.3.0 — Domain, Branding, Environment, and SaaS Foundation Planning (Step 4)

- Canonical source bumped to Master Source **v2.4.0**; added Step 4 planning requirements (§31).
- Domain/URL, brand, environment, configuration/secret, dependency, and SaaS Foundation planning requirements.
- No MVP scope change (§10); persona/pilot baseline (§28) unchanged. Product implementation: **NOT STARTED**.
- Evidence: `docs/domain/*`, `docs/brand/*`, `docs/environments/*`, `docs/dependencies/*`, `docs/planning/*`,
  `docs/quality/STEP_4_*`; ADRs 0033–0041; Claude rules 21–27; AFR-073..104.
