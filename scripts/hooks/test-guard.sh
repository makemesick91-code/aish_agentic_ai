...
run block "rm -rf /"
# Filesystem / device destruction and force-clean (must block).
run block "mkfs.ext4 /dev/sda1"
run block "mkfs -t ext4 /dev/sdb"
run block "dd if=/dev/zero of=/dev/sda"
run block "shred -u secret.txt"
run block "git clean -fd"
run block "git clean -fdx"
run block "git clean -xdf app/"
run block "git clean --force"
# Implementation-phase supply-chain safeguards (Step 5+): install ALLOWED, publish/deploy BLOCKED.
...
# Implementation-phase installs are allowed (Step 5+ supersedes the planning no-install safeguard).
run allow "composer install"
run allow "composer create-project laravel/laravel app"
run allow "npm install"
run allow "npm ci"
run allow "composer require laravel/framework"
