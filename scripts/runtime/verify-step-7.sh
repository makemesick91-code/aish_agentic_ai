#!/usr/bin/env bash
# verify-step-7.sh — prove the Step 7 Survey & CSAT foundation works against real PostgreSQL +
# Redis from a clean checkout, without regressing Step 5, Step 6, or SPRINT-SF-05.
#
# It migrates, re-runs the Step 6 SaaS-core and SF-05 real-infra checks (regression), then
# exercises the Step 7 survey foundation (immutable versioning, secure public invitation/token/
# QR, one-time tokenized responses, deterministic CSAT/NPS/CES, entitlement/usage, audit, and
# cross-tenant isolation) via `aish:verify-step-7`, and runs the hermetic test suite. Never
# prints secrets; uses only generated, non-sensitive data. Rule 29, rule 32.
set -uo pipefail
ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
cd "$ROOT"

EVID="docs/evidence/step-7/runtime"
mkdir -p "$EVID"
...
step "5. Step 7 survey & CSAT foundation (real infra)"
...
# Positive proof the survey checks actually ran (guards against a silently empty verifier).
if grep -q 'Step 7 verification passed' "$EVID/step-7.log"; then pass "Step 7 checks executed"; else bad "Step 7 checks did not report success"; fi

step "6. No secret VALUE leaked into verification logs"
...
step "7. Hermetic test suite"
if php artisan test >"$EVID/phpunit.log" 2>&1; then pass "php artisan test"; else bad "php artisan test (see $EVID/phpunit.log)"; fi

step "Summary"
if [ "$fail" -eq 0 ]; then echo "STEP 7 VERIFICATION: PASS (evidence in $EVID/)"; else echo "STEP 7 VERIFICATION: FAILED"; exit 1; fi
